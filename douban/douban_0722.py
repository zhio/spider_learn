...
def get_proxy_ip():
    ip_port = requests.get("http://localhost:8082/random").text
...
def crawl(urls,proxy = None,num_retries = 6):
    ...
    if proxy == None:
        try:
            ...
            if num_retries>0:
                print('获取网页出错,0S后获取倒数第',num_retries,'次')
        ...
        try:
            IP = get_proxy_ip()
            response = requests.get(urls, headers=headers, proxies=IP, timeout=5)
            data = response.json()['data']
            items = []
            for v in data:
                items.append(v)
...
def main():
    ...
    myclient = pymongo.MongoClient(host='localhost',username = 'admin',password = 'rootadmin', port=27017)
    mydb = myclient.test
